...
describe("profile and client initialization", () => {
  it("stores the secret only in the profile and merges client configuration", async () => {
    ...
    expect(text).toContain("sitepilot-mcp@0.1.6");
  ...
  it("writes only Antigravity's documented remote URL field and never emits an unsupported OAuth scope key", async () => {
    for (const client of ["antigravity-cli", "antigravity-ide"] as const) {
      ...
      expect(JSON.stringify(widened)).not.toContain('"oauth"');
    }
  });
  ...
  it("writes the complete detected major-client stdio matrix with an exact package pin", async () => {
    ...
    for (const server of [antigravityCli.mcpServers.sitepilot, windsurf.mcpServers.sitepilot]) {
      expect(server).toMatchObject({ command: "npx", args: ["-y", "sitepilot-mcp@0.1.6", "--profile", "example"], env: { SITEPILOT_URL: "https://example.com/" } });
